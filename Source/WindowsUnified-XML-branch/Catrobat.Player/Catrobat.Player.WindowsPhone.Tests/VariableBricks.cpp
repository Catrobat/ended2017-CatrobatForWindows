...
namespace PlayerWindowsPhone8Test
{
	...
	private:
		...
		shared_ptr<UserVariable> m_variable;
		FormulaTree *m_formulaTree;
		shared_ptr<Object> m_object;
		shared_ptr<StartScript> m_script;

		TEST_METHOD_INITIALIZE(TestInitialization)
		{
			m_defaultValue = "5";
			m_variableName = "testVariable";
			m_variable = shared_ptr<UserVariable>(new UserVariable(m_variableName, m_defaultValue));
			m_object = shared_ptr<Object>(new Object("TestObject"));
			m_script = shared_ptr<StartScript>(new StartScript(m_object));
			m_formulaTree = NULL;
		}
