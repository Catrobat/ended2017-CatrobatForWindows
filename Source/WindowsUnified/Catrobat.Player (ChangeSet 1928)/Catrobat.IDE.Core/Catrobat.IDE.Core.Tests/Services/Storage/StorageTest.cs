...
namespace Catrobat.IDE.Core.Tests.Services.Storage
{
    public class StorageTest : IStorage
    {
        ...
        {
            return await Task.Run(() => this.DirectoryExists(path));
        }

        public async Task<bool> FileExistsAsync(string path)
        {
            return await Task.Run(() => this.FileExists(path));
        }

        public async Task<string[]> GetDirectoryNamesAsync(string path)
        {
            return await Task.Run(() => this.GetDirectoryNames(path));
        }

        public async Task<string[]> GetFileNamesAsync(string path)
        {
            return await Task.Run(() => this.GetFileNames(path));
        }

        public async Task DeleteDirectoryAsync(string path)
        {
            await Task.Run(() => this.DeleteDirectory(path));
        }

        public async Task DeleteFileAsync(string path)
        {
            await Task.Run(() => this.DeleteFile(path));
        }

        public async Task CopyDirectoryAsync(string sourcePath, string destinationPath)
        {
            await Task.Run(() => this.CopyDirectory(sourcePath, destinationPath));
        }

        public async Task MoveDirectoryAsync(string sourcePath, string destinationPath)
        {
            await Task.Run(() => this.MoveDirectory(sourcePath, destinationPath));
        }

        public async Task CopyFileAsync(string sourcePath, string destinationPath)
        {
            await Task.Run(() => this.CopyFile(sourcePath, destinationPath));
        }

        public async Task MoveFileAsync(string sourcePath, string destinationPath)
        {
            await Task.Run(() => this.MoveFile(sourcePath, destinationPath));
        }
        ...
        {
            await Task.Run(() => this.RenameDirectory(directoryPath, newDirectoryName));
        }

        public async Task<PortableImage> LoadImageAsync(string pathToImage)
        {
            return await Task.Run(() => this.LoadImage(pathToImage));
        }

        public async Task<PortableImage> LoadImageThumbnailAsync(string pathToImage)
        {
            return await Task.Run(() => this.LoadImageThumbnail(pathToImage));
        }
