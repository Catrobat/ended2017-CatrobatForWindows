...
namespace Catrobat.Paint.WindowsPhone.Controls.UserControls
{
    public sealed partial class StampControl : UserControl
    {
        ...
        async public void StampImage()
        {
            ...
            if (currentApplication == null || tgPaintingAreaCheckeredGrid == null || !isSomethingDrawnOnWorkingSpace || !CheckIfStampSelctionIsChanged(currentApplication, tgPaintingAreaCheckeredGrid))
            {
                return;
            }

            Point currentLeftTopCoordinateOfStampSelection = GetXYOffsetBetweenPaintingAreaAndStampControlSelection();
            int heightOfStampSelection = (int)currentApplication.StampControl.GetHeightOfRectangleStampSelection();
            int widthOfStampSelection = (int)currentApplication.StampControl.GetWidthOfRectangleStampSelection();

            WriteableBitmap wbStampedBitmap = null;
            ...
            using (Stream streamOfSavedPictureFromWorkingSpace = await sfSavedPictureFromWorkingSpace.OpenStreamForReadAsync())
            {
                using (var memStream = new MemoryStream())
                {
                    ...
                    if (currentApplication.angularDegreeOfWorkingSpaceRotation == 0)
                    {
                        ...
                        bitmapBoundsOfDrawing.Height = (uint)heightOfStampSelection - 1;
                        bitmapBoundsOfDrawing.Width = (uint)widthOfStampSelection - 1;
                        uint uwidthOfStampSelection = (uint)widthOfStampSelection;
                        uint uheightOfStampSelection = (uint)heightOfStampSelection;
                        bitmapBoundsOfDrawing.X = (uwidthOfStampSelection + (uint)currentLeftTopCoordinateOfStampSelection.X) > canvasWidth ? canvasWidth - uwidthOfStampSelection : (uint)currentLeftTopCoordinateOfStampSelection.X;
                        bitmapBoundsOfDrawing.Y = ((uint)heightOfStampSelection + (uint)currentLeftTopCoordinateOfStampSelection.Y) > canvasHeight ? canvasHeight - uheightOfStampSelection : (uint)currentLeftTopCoordinateOfStampSelection.Y;
                        wbStampedBitmap = new WriteableBitmap(widthOfStampSelection, heightOfStampSelection);
                    }
                    else if (currentApplication.angularDegreeOfWorkingSpaceRotation == 90)
                    {
                        bitmapBoundsOfDrawing.Height = (uint)widthOfStampSelection - 1;
                        bitmapBoundsOfDrawing.Width = (uint)heightOfStampSelection - 1;
                        uint uwidthOfStampSelection = (uint)widthOfStampSelection;
                        uint uheightOfStampSelection = (uint)heightOfStampSelection;
                        bitmapBoundsOfDrawing.X = ((uint)heightOfStampSelection + (uint)currentLeftTopCoordinateOfStampSelection.X) > canvasWidth ? canvasWidth - uheightOfStampSelection : (uint)currentLeftTopCoordinateOfStampSelection.X;
                        bitmapBoundsOfDrawing.Y = ((uint)widthOfStampSelection + (uint)currentLeftTopCoordinateOfStampSelection.Y) > canvasHeight ? canvasHeight - uwidthOfStampSelection : (uint)currentLeftTopCoordinateOfStampSelection.Y;
                        wbStampedBitmap = new WriteableBitmap(widthOfStampSelection, heightOfStampSelection);
                    }
                    ...
                    //CommandManager.GetInstance().CommitCommand(new StampCommand(bitmapBoundsOfDrawing.X, bitmapBoundsOfDrawing.Y, (uint)(heightOfStampSelection - 1), (uint)(widthOfStampSelection - 1)));
                    ...
                    if (wbStampedBitmap != null)
                    {
                        wbStampedBitmap.SetSource(mrAccessStream);
                        AddWriteableBitmapToCanvas(wbStampedBitmap, currentApplication);
                    }
                }
            }
        }
